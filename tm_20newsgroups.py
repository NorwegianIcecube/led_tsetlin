...
target_words = ['atheist', 'christian', 'belief', 'faith', 'agnostic', 'hate', 'love']
...
enc = TMAutoEncoder(number_of_clauses=clauses, T=200, s=2, output_active=output_active)
...
for e in range(40):
    ...
    precision = []
    for i in range(len(target_words)):
        precision.append(enc.clause_precision(i, True, X_train_counts, number_of_examples=500))

    recall = []
    for i in range(len(target_words)):
        recall.append(enc.clause_recall(i, True, X_train_counts, number_of_examples=500))
